...
vUI.StyleFuncs["party"] = function(self, unit)
	-- General
	...
	vUI:SetFontInfo(HealthName, Settings["party-font"], Settings["party-font-size"], Settings["party-font-flags"])
	...
	if UF.BuffIDs[vUI.UserClass] then
		...
		for key, spell in pairs(UF.BuffIDs[vUI.UserClass]) do
...
vUI.StyleFuncs["partypet"] = function(self, unit)
	-- General
	self:RegisterForClicks("AnyUp")
	self:SetScript("OnEnter", UnitFrame_OnEnter)
	self:SetScript("OnLeave", UnitFrame_OnLeave)
...
local UpdatePartySpacing = function(value)
	if (Settings["party-orientation"] == "VERTICAL") then
		vUI.UnitFrames["party"]:SetAttribute("point", "BOTTOM")
		vUI.UnitFrames["party"]:SetAttribute("xoffset", 0)
		vUI.UnitFrames["party"]:SetAttribute("yoffset", value)
	else
		vUI.UnitFrames["party"]:SetAttribute("point", "LEFT")
		vUI.UnitFrames["party"]:SetAttribute("xoffset", value)
		vUI.UnitFrames["party"]:SetAttribute("yoffset", 0)
	end
end
...
local UpdateSetPartyOrientation = function(value)
	if value == "VERTICAL" then
		vUI.UnitFrames["party"]:SetAttribute("point", "TOP")
		vUI.UnitFrames["party"]:SetAttribute("xoffset", 0)
		vUI.UnitFrames["party"]:SetAttribute("yoffset", Settings["party-spacing"])
	else
		vUI.UnitFrames["party"]:SetAttribute("point", "LEFT")
		vUI.UnitFrames["party"]:SetAttribute("xoffset", Settings["party-spacing"])
		vUI.UnitFrames["party"]:SetAttribute("yoffset", 0)
	end
end
